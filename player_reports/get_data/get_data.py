...
def build_playerbox(year : int) -> pd.DataFrame:
    """
    ...
    ---------
        year: int
            The season year for which to build the player boxscore.
    ...
    data = sdv.mbb.load_mbb_player_boxscore(seasons=[year]).to_pandas()
    ...
    player_df = player_df[['teamId', 'full_team_name', 'jerseyNum', 'fullName', 'games', 'fgaPg', 'ftaRate', 'ftPct', 'fga3Rate', 'fg3Pct', 'fga3Pg', 'ftaPg', 'tovPg', 'orbPg', 'fg2Pct']].dropna()
...
def build_fourfacts(year : int) -> pd.DataFrame:
    ...
    opp_sos_df = build_opp_stats_df(data, team_stats)
